...
<body style="background-color: rgb(33, 24, 107);">
 ...
 ````````           </script> -->
  <!-- jQery -->
  <h1>Hello World!!!</h1>
    <button id="createBtn">Send</button><br><p>Your created room number is<span id="room"></span></p><br>
    <input type="text" placeholder="Enter your name" id="nameInput">
    <input type="text" placeholder="Enter room number" id="room_number">
    ...
    <script>
            ...
            var messageByOtherUsers="";
            var idOfOtherUsers="";
            var NameOfOtherUsers="";
            socket.on('createdRoom', room_num => {
                document.getElementById("room").innerHTML=room_num;
            })
            socket.on('RoomNotExists',() => {
                alert("This room not exists, You can create your!!!")
            })
            socket.on('message', message => {
                    messageByOtherUsers=message;
            })
            socket.on('userName', name => {
                NameOfOtherUsers=name;
            })
            socket.on('messageId', id =>{
                idOfOtherUsers=id;
                var allMessages = document.getElementById("AllMessages");
                const p =document.createElement("p");
                if (NameOfOtherUsers=="") {
                    p.innerHTML=idOfOtherUsers+": "+ messageByOtherUsers;
                }else{
                    p.innerHTML=NameOfOtherUsers+": "+ messageByOtherUsers;
                }
            ...
            sendBtn.addEventListener('click', (e) => {
                ...
                const list = [message,name,room];
                socket.emit("userMessage", list);
            });
